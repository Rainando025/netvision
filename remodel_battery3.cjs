...
if (index1 !== -1 && index2 !== -1) {
  ...
  body.position.y = height / 2; // Origin at bottom
  ...
  h1.position.set(-width/2 + 0.01, height * 0.7, 0);
  group.add(h1);
  const h2 = new THREE.Mesh(new THREE.BoxGeometry(0.1, 0.4, depth - 0.2), handleIndent);
  h2.position.set(width/2 - 0.01, height * 0.7, 0);
  group.add(h2);
