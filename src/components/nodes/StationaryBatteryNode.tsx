...
import type { StationaryBatteryNodeData } from "@/lib/types";
import { Battery, X } from "lucide-react";
import { useDiagram } from "@/lib/store";

export function StationaryBatteryNode({ id, data, selected }: { id: string; data: StationaryBatteryNodeData; selected?: boolean }) {
  ...
  return (
    ...
    >
      ...
      <button
        ...
        title="Remover Bateria Estacionária"
      >
        <X className="w-3 h-3" />
      </button>
      ...
      <div className="p-4 flex flex-col gap-3 group">
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1">
            ...
            <p className="text-[10px] text-muted-foreground uppercase tracking-wider">Bateria Estacionária</p>
          </div>
          <button
            onClick={(e) => { e.stopPropagation(); removeNode(id); }}
        ...
        <div className="flex gap-2">
          <div className="flex-1 bg-background/50 rounded p-2 text-center border border-border/50">
            ...
            <span className="text-xs font-medium text-foreground">{data.voltage}V</span>
          </div>
          <div className="flex-1 bg-background/50 rounded p-2 text-center border border-border/50">
            <span className="text-[10px] text-muted-foreground block mb-0.5">Capacidade</span>
            <span className="text-xs font-medium text-foreground">{data.capacityAh}Ah</span>
          </div>
        </div>
      </div>
